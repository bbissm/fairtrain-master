...
class table {
	...
	public function setup($config=[]) {
		...
		if($config["dragable"]!=null) {
			if(!is_array($this->tableClass)) $this->tableClass=[];
	...
	public function add($config) {
		if($this->maxCols<count($config["cols"])) $this->maxCols = count($config["cols"]);
		if(!is_array($config["style"])) $config["style"]=[];
	...
	public function addTitle($config) {
		...
		if(!is_array($config["class"])) $config["class"]=[];
	...
	public function addFormField($config) {
		...
		if($config["type"]=="datetime") {
			...
			// if($config["value"]=="01.01.1970 01:00") $config["value"]=date("d.m.Y H:i");
			...
			// if($config["value"]=="01.01.1970") $config["value"]=date("d.m.Y");
		}
	...
	public function auto($config) {
		...
		if($config["order"]!="") $order = "ORDER BY ".$config["order"];
		if($config["where"]!="") $where = "WHERE ".$config["where"];
		if(isset($config["controls"])) {
			$this->hasControls=1;
		}
		...
		while($res = $q->fetch_assoc()) {
			...
			if($this->maxCols<count($res)-1) $this->maxCols = count($res)-1;
	...
	public function render() { 
		...
		if($this->isForm) {
			...
			if($this->sqlTable!="") echo "<input type=\"hidden\" name=\"dformsubmit\" value=\"true\" />";
		}
		...
		foreach($this->table as $line) { 
			...
			foreach($line["cols"] as $key=>$col) {
				...
				if($key==count($line["cols"])-1) {
					if($this->maxCols-count($line["cols"])+1>1) $colspan = " colspan=\"".($this->maxCols-count($line["cols"])+1)."\""; 
				}
				...
				if($styletd1!="") $styletd1=" style=\"$styletd1\"";
